...
- `list_max(L, N)` - This relation is true if and only if `N` is the maximum value in `L`. Assume that the elements of `L` are all integers.
- `list_subset(L, M)` - This relation is true if and only if all the elements in list `M` are also members of list `L`.
