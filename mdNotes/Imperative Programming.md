...
Mutation in the context of programming is a fancy term that basically means change.
...
An equality a=b in math **declares** that some `a` is `b`, while an assignment operator `a=b` **commands** that `a`'s value is now the same as `b`.
Mutation is introduced once you perform an assignment to `a` again, signifying a **change** in the value of `a`.


By the way, the closest corresponding mathematical construct to an assignment statement is the let statement.
...
For every assignment statement you feed to the computer, something meaningful happens.
That particular "something" that happens is characterized by changes to your program's context.
The context of a program changes for every individual mutation of a variable.
...
An assignment statements effect to your computer is characterized by the progression from one state to another.
...
Creating meaningful programs in imperative programming is done by applying the Bohm-Jacopini Theorem.
...
A subprogram can be a single statement, or it can be a combination of more than one subprogram.
...
Rapaport W.
(2004) [Great Idea III: The Boehm-Jacopini Theorem and Structured Programming](https://cse.buffalo.edu/~rapaport/111F04/greatidea3.html).
[CSE 111, Fall 2004](http://www.cse.buffalo.edu/~rapaport/111F04.html) Accessed August 31, 2020
